...
bool UnitTest_Append(const char *filename1, const char *filename2)
{
  ...
  // save the mesh
  int startSave = clock();
  vcg::tri::io::ExporterSTL<MeshType>::Save(mr,filename2);
  int endSave = clock();
...
{
	//UnitTest_Append<MyMesh>(argv[1],argv[2]);
	UnitTest_Append<OcfMesh>(argv[1],argv[2]);
	return 0;
}
